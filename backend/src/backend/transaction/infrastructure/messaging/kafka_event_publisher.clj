(ns backend.transaction.infrastructure.messaging.kafka-event-publisher
  (:require [backend.transaction.application.port.out.event-publisher :as event-pub]
  ...
  (publish-event [_this event]
    (try
      (let [event-json (json/generate-string event)]
        (kafka/send! producer TRANSACTION_EVENTS_TOPIC (:id event) event-json)
